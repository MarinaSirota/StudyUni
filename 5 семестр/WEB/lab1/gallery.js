$(function(){
    $('.thumbnails a').click(function(){                                 // При нажатиина миниатюру
        ...
        let imgTitle=images.attr('title');

        $(".big-image img").attr({ src: imgSrc });
        $(".big-image img").attr({ title: imgTitle });                          // Подменяем адрес большого изображения на адрес выбранного
    ...
    $('.next').click(function(){                                         // При нажатии на кнопку "вперед"
        let count = $('.thumbnails a').length;                             // Общее количество изображений
        let n = parseInt($('.thumbnails a').index($('.active')) + 1);      // Порядковый номер текущего изображения
        let activeImg = $('.thumbnails .active');                          // Активное на данный момент изображение
        let nextSrc;

        if (count !== n){                                                   // - Если изображение не последнее
            nextSrc = activeImg.next().find('img').attr('src').attr('title');              // В переменную записывается адрес следующего изображения
            ...
            nextSrc = $('.thumbnails a').first().find('img').attr('src').attr('title');    // В переменную записывается адрес первого изображения
            $('.thumbnails .active').removeClass('active');                  // Удаляется класс .active с предыдущей миниатюры
    ...
    $('.prev').click(function(){                                         // При нажатии на кнопку "назад"
        ...
        if (n !== 1){                                                       // - Если текущее изображение не первое
            prevSrc = activeImg.prev().find('img').attr('src').attr('title');              // В переменную записывается адрес предыдущего изображения
            ...
            prevSrc = $('.thumbnails a:last').find('img').attr('src').attr('title');       // В переменную записывается адрес последнего изображения
            $('.thumbnails .active').removeClass('active');                  // Удаляется класс .active с предыдущей миниатюры
